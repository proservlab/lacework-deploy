...
output "target-gcp-instances" {
  value = [
    ...
    [
      for compute in gce.instances : compute.instances.network_interface[0].access_config[0].nat_ip if lookup(compute.instances.network_interface[0].access_config[0], "nat_ip", "false") != "false"
    ]
  ]
}
