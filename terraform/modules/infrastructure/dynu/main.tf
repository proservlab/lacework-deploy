resource "null_resource" "cname_dns_record" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
                set -e
                DYNU_DOMAIN_ID=$(curl -sLJ https://api.dynu.com/v2/dns -H "accept: application/json" -H "API-Key: ${ self.triggers.dynu_api_token }" | jq -r '.domains[] | select( .name=="${ self.triggers.dynu_dns_domain }" ) | .id ')
                DYNU_RECORD_ID=$(curl -sLJ https://api.dynu.com/v2/dns/$DYNU_DNS_ID/record -H "accept: application/json" -H "API-Key: ${ self.triggers.dynu_api_token }" | jq -r '.dnsRecords[] | select( .hostname=="${ each.key }.${ self.triggers.dynu_dns_domain }" ) | select( .recordType=="CNAME" ) | select( .state==true ) | .id')
                echo "Deleting Record $DYNU_RECORD_ID from Domain $DYNU_DOMAIN_ID."
                echo "Request: curl -sLJ -X DELETE https://api.dynu.com/v2/dns/$DYNU_DOMAIN_ID/record/$DYNU_RECORD_ID -H \"accept: application/json\""
...
resource "null_resource" "a_dns_record" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
                set -e
                DYNU_DOMAIN_ID=$(curl -sLJ https://api.dynu.com/v2/dns -H "accept: application/json" -H "API-Key: ${ self.triggers.dynu_api_token }" | jq -r '.domains[] | select( .name=="${ self.triggers.dynu_dns_domain }" ) | .id ')
                DYNU_RECORD_ID=$(curl -sLJ https://api.dynu.com/v2/dns/$DYNU_DNS_ID/record -H "accept: application/json" -H "API-Key: ${ self.triggers.dynu_api_token }" | jq -r '.dnsRecords[] | select( .hostname=="${ each.key }.${ self.triggers.dynu_dns_domain }" ) | select( .recordType=="A" ) | select( .state==true ) | .id')
                echo "Deleting Record $DYNU_RECORD_ID from Domain $DYNU_DOMAIN_ID."
                echo "Request: curl -sLJ -X DELETE https://api.dynu.com/v2/dns/$DYNU_DOMAIN_ID/record/$DYNU_RECORD_ID -H \"accept: application/json\""
                curl -sLJ -X DELETE https://api.dynu.com/v2/dns/$DYNU_DOMAIN_ID/record/$DYNU_RECORD_ID -H "accept: application/json" -H "API-Key: ${ self.triggers.dynu_api_token }"
                EOT
