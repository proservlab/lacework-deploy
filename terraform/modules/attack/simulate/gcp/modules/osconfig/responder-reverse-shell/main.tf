...
resource "google_os_config_os_policy_assignment" "osconfig-responder-reverse-shell" {

  project     = var.gcp_project_id
  location    = data.google_compute_zones.available.names[0]
  
  name        = "osconfig-responder-reverse-shell-${var.environment}-${var.deployment}"
  description = "Responder reverse shell"
  ...
  os_policies {
    id   = "osconfig-responder-reverse-shell-${var.environment}-${var.deployment}"
    mode = "ENFORCEMENT"
