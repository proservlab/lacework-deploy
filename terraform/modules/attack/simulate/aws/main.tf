...
module "ssm-connect-codecov" {
  count = (var.config.context.global.enable_all == true) || (var.config.context.global.disable_all != true && local.target == true && var.config.context.aws.ssm.target.connect.codecov.enabled == true) ? 1 : 0
...
module "ssm-execute-docker-log4shell-attack" {
  count = (var.config.context.global.enable_all == true) || (var.config.context.global.disable_all != true && local.attacker == true && var.config.context.aws.ssm.attacker.execute.docker_log4shell_attack.enabled == true) ? 1 : 0
...
module "ssm-listener-port-forward" {
  count = (var.config.context.global.enable_all == true) || (var.config.context.global.disable_all != true && local.attacker == true && var.config.context.aws.ssm.target.listener.port_forward.enabled == true) ? 1 : 0
