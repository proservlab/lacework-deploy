...
resource "aws_ssm_document" "exec_docker_host_cryptomining_attacker" {
  name          = "exec_docker_host_cryptomining_${var.environment}_${var.deployment}"
        ...
        "mainSteps": [
            {
                ...
                "name": "exec_docker_host_cryptomining_attacker_${var.environment}_${var.deployment}",
                "precondition": {
                    "StringEquals": [
                        "platformType",
                        "Linux"
...
resource "aws_resourcegroups_group" "exec_docker_host_cryptomining_attacker" {
    name = "exec_docker_host_cryptomining_${var.environment}_${var.deployment}"

    resource_query {
        query = jsonencode(var.resource_query_exec_docker_host_cryptomining_attacker)
    }
...
resource "aws_ssm_association" "exec_docker_host_cryptomining_attacker" {
    association_name = "exec_docker_host_cryptomining_${var.environment}_${var.deployment}"

    name = aws_ssm_document.exec_docker_host_cryptomining_attacker.name

    targets {
        key = "resource-groups:Name"
        values = [
            aws_resourcegroups_group.exec_docker_host_cryptomining_attacker.name,
        ]
    }
