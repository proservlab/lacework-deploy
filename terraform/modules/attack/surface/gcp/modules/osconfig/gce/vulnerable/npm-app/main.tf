...
resource "google_os_config_os_policy_assignment" "exec-vuln-npm-app" {

  project     = var.gcp_project_id
  location    = data.google_compute_zones.available.names[0]
  
  name        = "osconfig-exec-vuln-npm-app-${var.environment}-${var.deployment}"
  ...
  os_policies {
    id   = "osconfig-exec-vuln-npm-app-${var.environment}-${var.deployment}"
    mode = "ENFORCEMENT"
