...
module "ssh-keys" {
  ...
  ssh_private_key_path = local.config.context.gcp.osconfig.ssh_keys.ssh_private_key_path
  ssh_authorized_keys_path = local.config.context.gcp.osconfig.ssh_keys.ssh_authorized_keys_path
}
...
module "vulnerable-log4j-app" {
  ...
  listen_port = local.config.context.gcp.osconfig.vulnerable.log4j_app.listen_port
}
