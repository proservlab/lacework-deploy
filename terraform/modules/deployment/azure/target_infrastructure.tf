...
module "target-azuresql" {
  ...
  add_service_principal_access        = try(length(local.target_infrastructure_config.context.azure.azuresql.service_principal_name), "false") != "false" ? true : false
  service_principal_display_name      = try(length(local.target_infrastructure_config.context.azure.azuresql.service_principal_name), "false") != "false" ?  module.target-iam[0].service_principal_ids[local.target_infrastructure_config.context.azure.azuresql.service_principal_name].display_name : null

  mysql_authorized_ip_ranges          = local.target_infrastructure_config.context.azure.azuresql.instance_type == "mysql" ? [ for ip in flatten([
  ...
  postgres_authorized_ip_ranges        = local.target_infrastructure_config.context.azure.azuresql.instance_type == "postgres" ? [ for ip in flatten([
    [ for compute in try(local.public_attacker_instances, []): "${compute.public_ip}" ],
    [ for compute in try(local.public_attacker_app_instances, []): "${compute.public_ip}" ],
    [ for compute in try(local.public_target_instances, []): "${compute.public_ip}" ],
    [ for compute in try(local.public_target_app_instances, []): "${compute.public_ip}" ],
