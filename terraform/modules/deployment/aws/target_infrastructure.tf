...
locals {
  ...
  target_cluster_name                         = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster.id : null
  target_cluster_endpoint                     = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster.endpoint : null
  target_cluster_ca_cert                      = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster.certificate_authority[0].data : null
  target_cluster_oidc_issuer                  = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster.identity[0].oidc[0].issuer : null
  target_cluster_security_group               = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster_sg_id : null
  target_cluster_subnet                       = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster_subnet : null
  target_cluster_vpc_id                       = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster_vpc_id : null
  target_cluster_node_role_arn                = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster_node_role_arn : null
  target_cluster_vpc_subnet                   = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster_vpc_subnet : null
  target_cluster_openid_connect_provider_arn  = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster_openid_connect_provider.arn : null
  target_cluster_openid_connect_provider_url  = local.target_infrastructure_config.context.aws.eks.enabled ? module.target.target-eks[0].cluster_openid_connect_provider.url : null
...
# inspector
module "target-inspector" {
  count = (local.target_infrastructure_config.context.global.enable_all == true) || (local.target_infrastructure_config.context.global.disable_all != true && local.target_infrastructure_config.context.aws.inspector.enabled == true ) ? 1 : 0
