...
module "attacker-lacework-custom-policy" {
  count = (local.attacker_infrastructure_config.context.global.enable_all == true) || (local.attacker_infrastructure_config.context.global.disable_all != true && local.attacker_infrastructure_config.context.lacework.custom_policy.enabled == true ) ? 1 : 0
...
module "attacker-lacework-alerts" {
  count = (local.attacker_infrastructure_config.context.global.enable_all == true) || (local.attacker_infrastructure_config.context.global.disable_all != true && local.attacker_infrastructure_config.context.lacework.alerts.enabled == true ) ? 1 : 0
  source       = "../common/lacework-alerts"
  environment = local.attacker_infrastructure_config.context.global.environment
  deployment   = local.attacker_infrastructure_config.context.global.deployment

  enable_slack_alerts = local.attacker_infrastructure_config.context.lacework.alerts.slack.enabled
  slack_token = local.attacker_infrastructure_config.context.lacework.alerts.api_token
  enable_jira_cloud_alerts = local.attacker_infrastructure_config.context.lacework.alerts.jira.enabled
  jira_cloud_url = local.attacker_infrastructure_config.context.lacework.alerts.jira.cloud_url
  jira_cloud_project_key = local.attacker_infrastructure_config.context.lacework.alerts.jira.cloud_project_key
  jira_cloud_issue_type = local.attacker_infrastructure_config.context.lacework.alerts.jira.cloud_issue_type
  jira_cloud_username = local.attacker_infrastructure_config.context.lacework.alerts.jira.cloud_username
