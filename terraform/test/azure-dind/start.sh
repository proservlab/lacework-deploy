...
if docker ps | grep deploy-azure-dind; then
    echo "Found running container, attaching..."
    docker attach deploy-azure-dind
elif docker ps -a | grep deploy-azure-dind;then
    echo "Found stopped container, starting and attaching..."
    docker start deploy-azure-dind
    docker attach deploy-azure-dind
else
    echo "Starting and attaching to container..."
    sudo docker run -d --privileged -v /var/run/docker.sock:/var/run/docker.sock --name=deploy-azure-dind -w /workspace/terraform -it $MOUNT deploy-azure-dind:latest \
    && docker attach deploy-azure-dind
fi;
