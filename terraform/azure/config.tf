...
locals {
  attacker-infrastructure-config-file = templatefile(
    "${var.scenarios_path}/${var.scenario}/attacker/infrastructure.json",
    {
      ...
      # azure
      azure_tentant      = can(length(var.attacker_azure_tenant)) ? var.attacker_azure_tenant : ""
  ...
  target-infrastructure-config-file = templatefile(
    "${var.scenarios_path}/${var.scenario}/target/infrastructure.json",
    {
      ...
      # azure
      azure_tentant      = can(length(var.target_azure_tenant)) ? var.target_azure_tenant : ""
  ...
  attacker_attacksurface_override = {
    context = {
      azure = {
        compute = {
          add_trusted_ingress = {
            enabled = length([for x in local.attacker_infrastructure_temp_config["context"]["azure"]["compute"]["instances"] : x if x["public"] == true && x["role"] == "default"]) > 0 ? try(local.attacker_attacksurface_temp_config["context"]["azure"]["compute"]["add_trusted_ingress"]["enabled"], false) : false
          }
          add_app_trusted_ingress = {
            enabled = length([for x in local.attacker_infrastructure_temp_config["context"]["azure"]["compute"]["instances"] : x if x["public"] == true && x["role"] == "app"]) > 0 ? try(local.attacker_attacksurface_temp_config["context"]["azure"]["compute"]["add_app_trusted_ingress"]["enabled"], false) : false
          }
        }
      }
    }
  ...
  target_attacksurface_override = {
    context = {
      azure = {
        compute = {
          add_trusted_ingress = {
            enabled = length([for x in local.target_infrastructure_temp_config["context"]["azure"]["compute"]["instances"] : x if x["public"] == true && x["role"] == "default"]) > 0 ? try(local.target_attacksurface_temp_config["context"]["azure"]["compute"]["add_trusted_ingress"]["enabled"], false) : false
          }
          add_app_trusted_ingress = {
            enabled = length([for x in local.target_infrastructure_temp_config["context"]["azure"]["compute"]["instances"] : x if x["public"] == true && x["role"] == "app"]) > 0 ? try(local.target_attacksurface_temp_config["context"]["azure"]["compute"]["add_app_trusted_ingress"]["enabled"], false) : false
          }
        }
      }
    }
...
locals {
  attacker-attacksimulation-config-file = templatefile(
    "${var.scenarios_path}/${var.scenario}/shared/simulation.json",
    {
      ...
      # azure
      attacker_azure_tentant      = can(length(var.attacker_azure_tenant)) ? var.attacker_azure_tenant : ""
      attacker_azure_subscription = var.attacker_azure_subscription
      attacker_azure_region       = var.attacker_azure_region
      target_azure_tentant        = can(length(var.target_azure_tenant)) ? var.target_azure_tenant : ""
